...
for study in studies:
    ...
    for seq in data.columns[:-1]:
        ...
        diff_ex[(study, seq)] = (np.sign(cancers[seq].mean() - healthy[seq].mean()), ttest_ind(cancers[seq], healthy[seq]).pvalue < 0.05)
...
for s1, s2 in tqdm(list(combinations(studies, 2))):
    ...
    if len(mirna_ps) < 20:
        continue
    mirna_ps.sort()
    most_consistent.extend(diff_ex[(s1, seq)][0] == diff_ex[(s2, seq)][0] for _, seq in mirna_ps[:10] if diff_ex[(s1, seq)][1] == diff_ex[(s2, seq)][1] == 1)
    least_consistent.extend(diff_ex[(s1, seq)][0] == diff_ex[(s2, seq)][0] for _, seq in mirna_ps[-10:] if diff_ex[(s1, seq)][1] == diff_ex[(s2, seq)][1] == 1)
...
df.to_csv(get_project_path() / "Outdata" / "signed_rank_cv_significant.csv", index=False)
