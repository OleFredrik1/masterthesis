...
def mirbase_to_seq(mirna_list, ignore_not_found=False):
    ...
    lookup_table = {}
    with open(get_project_path() / "Others" / "mature.fa", "r") as f:
        for line in f:
...
def get_seq_lookup_table():
    ...
    with open(get_project_path() / "Others" / "mature.fa", "r") as f:
        for line in f:
            ...
            line2 = next(f)
            lookup_table[line2.strip()] = name
    return lookup_table

def seq_to_mirbase(mirna_list):
    lookup_table = get_seq_lookup_table()
    seq = [lookup_table[s] for s in mirna_list]
    return seq

def canonical_to_mirbase(mirna_list, ignore_not_found=False):
    lookup_table = {}
    from Utils.config import get_project_path
    with open(get_project_path() / "Others" / "aliases.txt", "r") as f:
        for line in f:
            line = line.strip()[:-1]
